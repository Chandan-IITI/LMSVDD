function alphas = smo_solver(alphas, C, epsilon, labels, linears, quadratics, tau)
    gradients = linears;
    while 1
        [i, j] = working_set_selection(alphas, C, epsilon, gradients, labels, quadratics, tau);
        if (j == -1)
           break;
        end
...
function [i, j] = working_set_selection(alphas, C, epsilon, gradients, labels, quadratics, tau)
    N = size(alphas, 1);
    i = -1;
    maxGradient = -Inf;
    minGradient = +Inf;
    for k = 1:N
        if (labels(k) == +1 && alphas(k) < C) || (labels(k) == -1 && alphas(k) > 0)
    ...
    minObjective = +Inf;
    for k = 1:N
        if (labels(k) == +1 && alphas(k) > 0) || (labels(k) == -1 && alphas(k) < C)
